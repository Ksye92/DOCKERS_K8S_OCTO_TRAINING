...
search ksaidani.svc.cluster.local svc.cluster.local cluster.local
options ndots:5
/ # exit
```
...
```sh
search ksaidani.svc.cluster.local [...]
```
...
Vous trouverez ci-dessous un exemple d’une ressource de type ***deployment***. Créez un fichier `deployment-kubernetes-app.yaml` (dans le dossier  `~/ksaidani/deployment/`) et copiez-y le code suivant :
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: kubernetes-app
        image: 261407191094.dkr.ecr.eu-west-3.amazonaws.com/ksaidani/app:v0.1
        ports:
        - containerPort: 8000
          name: http
```
